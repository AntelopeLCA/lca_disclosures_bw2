...
import functools
import warnings
from lca_disclosures.utils import data_to_rcv
...
class DisclosureImporter(LCIImporter):
    ...
    ::
        json dictionary with the following keys:
            'Af':{
                'data': [COO data describing foreground (Af) matrix in the format [[row, column], value]]
                'shape': shape as a 2 item list [rows, columns]
            },
            'Ad':{
                'data': [COO data describing background (Ad) matrix in the format [[row, column], value]]
                'shape': shape as a 2 item list [rows, columns]
            },
            'Bf':{
                'data': [COO data describing foreground emissions (Bf) matrix in the format [[row, column], value]]
                'shape': shape as a 2 item list [rows, columns]
            },
            'foreground flows': [ # list of dictionaries representing the indexed foreground flows in Af
                {
                    #  'index': Af index as integer  ## removed-- index is implicit
                    'location': as string,
                    'name': 'as string,
                    'unit': as string
                },
            ...
            'background flows': [ # list of dictionaries representing the indexed background flows in Ad
                {
                    ...
                    #  'index': Ad row index as integer,  ## removed-- index is implicit
                    'location': as string,
                    'unit': as string
                },
                ]
            'foreground emissions': [ # list of dictionaries representing the indexed foreground emissions in Bf
                {
                    ...
                    #  'index': as integer,  ## removed-- index is implicit
                    'name': as string,
                    'unit': as string
                },
                ]
    ...
    def __init__(self, filepath, db_name="Disclosure_database"):
        super(DisclosureImporter, self).__init__(db_name)
    ...
    def match_required_databases(self, data):
        for db in self.required_databases:
            if db in databases:
                ...
                warnings.warn('Database "{}" does not exist in the current project, create/import this '
                              'database and try again using '
                              '<DisclosureImporter_instance>.apply_strategies()'.format(db))
        return data
